<template>
  <a-config-provider :locale="zhCN" :theme="{
    ...
    <a-layout :style="{
      ...
      <Footer v-if="userStore.token" />
      <a-float-button-group shape="circle" trigger="hover" type="primary" class="!right-5 !bottom-20">
        <template #icon>
          <SettingFilled />
        </template>
        <a-float-button @click="toggleDark" type="primary">
          <template #icon>
...
import Icon, { SettingFilled } from '@ant-design/icons-vue';
